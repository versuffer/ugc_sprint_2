...
class UgcService:
    ...
    async def get_score(film_id: UUID) -> ScoreSchema:
        avg_score = await ScoreModel.find(ScoreModel.film_id == film_id).avg(
            ScoreModel.score
        )
    ...
    async def upsert_score(
        user_id: UUID, film_id: UUID, score: int
    ) -> CreateScoreSchema:
        upsert_document = {
            "user_id": user_id,
            "film_id": film_id,
        ...
        if user_film_score := await ScoreModel.find_one(
            ScoreModel.film_id == film_id, ScoreModel.user_id == user_id
        ):
    ...
    async def get_bookmarks(user_id: UUID) -> list[BookmarkSchema]:
        ...
        user_bookmarks = await BookmarkModel.find(
            BookmarkModel.user_id == user_id
        ).to_list()

        return [
            BookmarkSchema.model_validate(bookmark.model_dump())
    ...
    async def add_text_review(
        ...
        user_film_text_review = await TextReviewModel(
            **text_review_data.model_dump()
        ).insert()
        return TextReviewSchema.model_validate(user_film_text_review.model_dump())

    @staticmethod
    async def upsert_score_review(
        review_id: UUID, score_review_data: CreateScoreReviewSchema
    ) -> ScoreReviewSchema:
        upsert_data = score_review_data.model_dump() | {"review_id": review_id}

        if review_score := await ScoreReviewModel.find_one(
            ScoreReviewModel.review_id == review_id
        ):
    ...
    async def get_film_reviews(film_id: UUID) -> list[ReviewExtSchema]:
        film_text_reviews = await TextReviewModel.find(
            TextReviewModel.film_id == film_id
        ).to_list()
        return [
            ReviewExtSchema.model_validate(film_text_review.model_dump())
            for film_text_review in film_text_reviews
        ]
